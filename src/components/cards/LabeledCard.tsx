...
export default function LabeledCard({label, slotProps, children}: PropsWithChildren<LabeledCardProps>) {
    return <div className='flex flex-col items-center'>
        <div className={`border-qetra-success-border border-2 rounded-full w-max p-4 uppercase text-qetra-success relative top-4 ${slotProps?.label?.className ?? ''}`}>{label}</div>
